...
namespace NoisetteCore.Obfuscation
{
    public class ObfuscationProcess
    {
        ...
        public void DoObfusction()
        {
            ...
            Protection.AntiTampering.AntiTamperingProtection.Md5(Path.GetDirectoryName(_module.Location) + @"\i.dll");
        }

        public void SaveAssembly()
        {
            Core.Property.opts.Logger = DummyLogger.NoThrowInstance;
            _module.Write(Path.GetDirectoryName(_module.Location) + @"\i.dll", Core.Property.opts);
            CH.Debug("File Saved ! ");
            CH.Finish();
        }
    }
