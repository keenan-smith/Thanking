			...
			if (Provider.modeConfigData.Gameplay.Ballistics)
			{
				for (int i = 0; i < Bullets.Count; i++)
				{
					...
					if (RaycastOptions.Enabled)
						ri = RaycastUtilities.GenerateRaycast();
					else
						ri = RaycastUtilities.GenerateOriginalRaycast(new Ray(Player.player.look.aim.position, Player.player.look.aim.forward), PAsset.range, RayMasks.DAMAGE_CLIENT);
